...
# Fix duplicate PRIMARY KEY definitions in all files that have 'AUTO_INCREMENT PRIMARY KEY'
for f in V3__Create_Commission_Table V4__Create_Binary_Pool_Table V5__Create_Rank_History_Table V6__Create_Epin_Table V7__Create_Agent_Tables V8__Create_Payment_Table V9__Create_Academic_Event_Table V10__Create_Audit_Log_Table V11__Create_Withdrawal_Table V12__Create_Config_Table V13__Create_Document_Table; do
  # Remove lines that are just "    PRIMARY KEY (id)," (redundant with AUTO_INCREMENT PRIMARY KEY)
  sed -i '/^    PRIMARY KEY (id),$/d' "${f}.sql"
  echo "Fixed ${f}.sql"
done
...
for f in V3__Create_Commission_Table V4__Create_Binary_Pool_Table V5__Create_Rank_History_Table V6__Create_Epin_Table V7__Create_Agent_Tables V8__Create_Payment_Table V9__Create_Academic_Event_Table V10__Create_Audit_Log_Table V11__Create_Withdrawal_Table V12__Create_Config_Table V13__Create_Document_Table; do
  echo "=== Running ${f} ==="
  mysql -uroot -p'6qZB6d@pIvj' temco_system < "${f}.sql" 2>&1 | grep -iE 'error|fail' | head -3
done

echo "=== Running V15__Add_Circular_FKs ==="
mysql -uroot -p'6qZB6d@pIvj' temco_system < V15__Add_Circular_FKs.sql 2>&1 | grep -iE 'error|fail' | head -3
...
echo "=== Verifying all EEPIP tables ==="
mysql -uroot -p'6qZB6d@pIvj' temco_system -e "SHOW TABLES LIKE 'eepip%'"
